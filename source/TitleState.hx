...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// https://github.com/HaxeFlixe<l/flixel-addons/pull/348
		...
		blackScreen = new FlxSprite().loadGraphic(Paths.image('TitleIntroBG'));
			...
			if(pressedEnter)
			{
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					if (ClientPrefs.warnings) {
						MusicBeatState.switchState(new WarningIntroState());
					} else {
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// credTextShit.visible = true;
				case 14:
					addMoreText('Susbor Editiom');
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('Alpha 1.1'); // credTextShit.text += '\nFunkin';
