...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['YOU FUCKING LIAR MOLDY!', 0.2], //From 0% to 19%
		['Your bad at this game', 0.4], //From 20% to 39%
		['YOU LIAR', 0.5], //From 40% to 49%
		['Holy Shit', 0.6], //From 50% to 59%
		['Phone Phone Phone', 0.69], //From 60% to 68%
		['Fuck idiot *lag*', 0.7], //69%
		['I fucking knew it MOLDY!', 0.8], //From 70% to 79%
		['Never coming back again', 0.9], //From 80% to 89%
		['Stop calling me', 1], //From 90% to 99%
		['IM GONNA BREAK MY PHONE!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	var watermarkTxtTween:FlxTween;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'spombors': //Spombori
				defaultCamZoom = 0.85;
				curStage = 'spombors';
				var bg:BGSprite = new BGSprite('bambi/spombors/stageback', -600, -200, 0.9, 0.9);
			...
			case 'samsung': //Samsung
				defaultCamZoom = 0.85;
				curStage = 'samsung';
				var bg:BGSprite = new BGSprite('bambi/samsung/stageback', -600, -200, 0.9, 0.9);
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
			...
			case 'samsung-pibby': //Samsung
				defaultCamZoom = 0.85;
				curStage = 'samsung-pibby';
				var bg:BGSprite = new BGSprite('bambi/samsung/stagebackpibby', -600, -200, 0.9, 0.9);
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
			...
			case 'samsung-unfair': //Samsung
				defaultCamZoom = 0.85;
				curStage = 'samsung-unfair';
				var bg:BGSprite = new BGSprite('bambi/samsung/stagebackunfair', -600, -200, 0.9, 0.9);
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
			...
			case 'susnovae': //Samsung
				defaultCamZoom = 0.85;
				curStage = 'susnovae';
				var bg:BGSprite = new BGSprite('bambi/wavyjokes/susnovae', -600, -200, 0.9, 0.9);
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
			...
			case 'susnovae-2': //Samsung
				defaultCamZoom = 0.85;
				curStage = 'susnovae-2';
				var bg:BGSprite = new BGSprite('bambi/wavyjokes/susnovae2', -600, -200, 0.9, 0.9);
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
			...
			case 'dave-house':
				...
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('bambi/house/sky'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.75, 0.75);
				bg.active = false;
				add(bg);
	
				var stageHills:FlxSprite = new FlxSprite(-225, -125).loadGraphic(Paths.image('bambi/house/hills'));
				...
				var gate:FlxSprite = new FlxSprite(-200, -125).loadGraphic(Paths.image('bambi/house/gate'));
				...
				var stageFront:FlxSprite = new FlxSprite(-225, -125).loadGraphic(Paths.image('bambi/house/grass'));
		...
		if(ClientPrefs.timeBarType == 'Song Name and Time Left' || ClientPrefs.timeBarType == 'Song Name and Time Elapsed')
		{
		...
		if(SONG.credit != null) {
			if (SONG.screwYou != null) {
				watermarkTxt = new FlxText(0, healthBarBG.y + 34, FlxG.width, SONG.song + " - " + CoolUtil.difficultyString() + " - VDAB Susbor Edition | Credit to " + SONG.credit + " - Inspired by Joolian\n" + SONG.screwYou + "", 20);
				watermarkTxt.setFormat(Paths.font("comic.ttf"), 15, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				...
				watermarkTxt = new FlxText(0, healthBarBG.y + 34, FlxG.width, SONG.song + " - " + CoolUtil.difficultyString() + " - VDAB Susbor Edition | Credit to " + SONG.credit + " - Inspired by Joolian", 20);
				watermarkTxt.setFormat(Paths.font("comic.ttf"), 15, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				watermarkTxt.scrollFactor.set();
				watermarkTxt.borderSize = 1.25;
				watermarkTxt.visible = !ClientPrefs.hideHud;
			...
			if (SONG.screwYou != null) {
				watermarkTxt = new FlxText(0, healthBarBG.y + 34, FlxG.width, SONG.song + " - " + CoolUtil.difficultyString() + " - VDAB Susbor Edition | Inspired by Joolian\n" + SONG.screwYou + "", 20);
				watermarkTxt.setFormat(Paths.font("comic.ttf"), 15, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				watermarkTxt.scrollFactor.set();
				watermarkTxt.borderSize = 1.25;
				watermarkTxt.visible = !ClientPrefs.hideHud;
				...
				watermarkTxt = new FlxText(0, healthBarBG.y + 34, FlxG.width, SONG.song + " - " + CoolUtil.difficultyString() + " - VDAB Susbor Edition | Inspired by Joolian", 20);
				watermarkTxt.setFormat(Paths.font("comic.ttf"), 15, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				watermarkTxt.scrollFactor.set();
				watermarkTxt.borderSize = 1.25;
				watermarkTxt.visible = !ClientPrefs.hideHud;
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "CHEATING MOMENT", 32);
		...
		if(ClientPrefs.hitsoundType == 'vs-dave') {
			if(ClientPrefs.hitsoundVolume > 0) 
				CoolUtil.precacheSound('note_click');
		}
		if(ClientPrefs.hitsoundType == 'osu!mania') {
			if(ClientPrefs.hitsoundVolume > 0) 
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			...
			scoreTxt.text = 'Score: ' + songScore + '\nMisses: ' + songMisses + '\nCombos: ' + songHits + '\nHealth: ' + health + '%\nRating: ' + ratingName + '\nAccuracy: ' + Math.floor(ratingPercent * 100) + '% - ' + ratingFC + '\n\nSicks : ' + sicks + '\nGoods : ' + goods + '\nBads : ' + bads + '\nShits : ' + shits + '\n\n';//peeps wanted no integer rating
		}
		...
		else
		{
			...
			if (!paused)
			{
				...
				if(updateTime) {
					...
					if(ClientPrefs.timeBarType == 'Time Elapsed' || ClientPrefs.timeBarType == 'Song Name and Time Elapsed') songCalc = curTime;
		...
		if(curSong.toLowerCase() == 'spomborness')
		{
					...
					switch (curSong.toLowerCase())
					{
						case 'loseness':
							if (health >= 0.1)
							{
								...
								MusicBeatState.switchState(new EndingGoodState());
							...
							else if (songMisses >= 500)
							{
								...
								MusicBeatState.switchState(new EndingBadState());
	
								changedDifficulty = false;
							}
							else if (ClientPrefs.getGameplaySetting('practice', true))
							{
								FlxG.sound.playMusic(Paths.music('freakyMenu'));
								...
								MusicBeatState.switchState(new EndingBad2State());
							...
							default:
								...
								MusicBeatState.switchState(new StorySelectorState());
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(!practiceMode && !cpuControlled) {
			...
			if(ClientPrefs.scoreZoom)
			{
				...
				if(watermarkTxtTween != null) {
					watermarkTxtTween.cancel();
				}
				watermarkTxt.scale.x = 1.1;
				watermarkTxt.scale.y = 1.1;
				watermarkTxtTween = FlxTween.tween(watermarkTxt.scale, {x: 1, y: 1}, 0.2, {
					onComplete: function(twn:FlxTween) {
						watermarkTxtTween = null;
					}
				});
			}
		}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			if(ClientPrefs.hitsoundType == 'vs-dave') {
				...
				{
					FlxG.sound.play(Paths.sound('note_click'), ClientPrefs.hitsoundVolume);
				}
			}
