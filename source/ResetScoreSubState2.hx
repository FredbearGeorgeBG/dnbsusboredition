...
class ResetScoreSubState2 extends MusicBeatSubstate
{
	var bg:FlxSprite;
	...
	public function new(song:String, difficulty:Int, character:String, week:Int = -1)
	{
		...
		if(week > -1) {
			name = WeekData3.weeksLoaded.get(WeekData3.weeksList[week]).weekName;
		}
		...
		if(controls.BACK) {
			...
			if(onYes) {
				if(week == -1) {
					...
					Highscore.resetWeek(WeekData3.weeksList[week], difficulty);
				}
			}
			FlxG.sound.play(Paths.sound('cancelMenu'), 1);
			close();
